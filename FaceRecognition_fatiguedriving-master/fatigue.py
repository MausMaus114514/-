...
class FatigueDataSimulator:
    ...
    def simulate_to_file(self, output_path: str, interval: float = 2.0, max_records: int = None):
        """
        持续生成模拟数据并写入文件
        :param output_path: 输出文件路径
        :param interval: 生成间隔(秒)
        :param max_records: 最大生成条数，None表示无限
        """
        count = 0
        ...
        try:
            while True:
                ...
                # 控制台输出
                status_color = "\033[32m" if data["status_code"] == 0 else \
                              "\033[33m" if data["status_code"] == 1 else "\033[31m"
                print(f"{status_color}[{data['timestamp']}] 状态: {data['status_text']} "
                      f"(眨眼: {data['blink_count']}, 哈欠: {data['yawn_count']}, 点头: {data['head_nod_count']})\033[0m")
...
if __name__ == "__main__":
    ...
    # 每2秒生成一条数据，无限生成
    simulator.simulate_to_file(output_file, interval=2.0)