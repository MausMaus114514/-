...
#modify here, can be justify if needed
...
#modify part end
        ...
        self.setupUi(self)
        # 添加后端API配置
        self.BACKEND_URL = "http://localhost:5000"  # 后端地址py
        self.API_UPLOAD = f"{self.BACKEND_URL}/api/upload"  # 上传API
        self.last_upload_time = {}  # 记录上次上传时间，避免频繁上传
        # 连接信号
    ...
    def send_to_backend(self, fatigue_level):
        """发送疲劳数据到后端"""
        try:
            # 避免频繁发送相同状态
            current_time = datetime.datetime.now()
            if fatigue_level in self.last_upload_time:
                if (current_time - self.last_upload_time[fatigue_level]).seconds < 10:
                    return  # 10秒内不重复发送相同状态

            # 准备数据
            data = {
                "user_id": "1",  # 用户ID，可以根据实际情况修改
                "fatigue_level": fatigue_level,  # 0:正常, 1:预警, 2:警报
                "device_info": "Camera"  # 设备信息
            }

            # 发送请求
            response = requests.post(self.API_UPLOAD, json=data)

            if response.status_code == 201:
                self.plainTextEdit_tip.appendPlainText(f"数据上传成功: {fatigue_level}")
            else:
                self.plainTextEdit_tip.appendPlainText(f"上传失败: {response.text}")

            # 更新最后上传时间
            self.last_upload_time[fatigue_level] = current_time

        except Exception as e:
            self.plainTextEdit_tip.appendPlainText(f"上传异常: {str(e)}")
    ...
    def paly_sound(self):
        if self.sound_thread is not None and self.sound_thread.is_alive():
            ...
            self.plainTextEdit_tip.appendPlainText('疲劳驾驶')
    ...
    def thread_singnal_slot(self, d):
        if d['type']=='img':
            ...
            if d['value'][1] == '轻微疲劳':
                ...
                self.send_to_backend(1)  # 1 表示预警
            elif  d['value'][1] == '瞌睡':
                self.label_12.setStyleSheet("color:red;")
                self.send_to_backend(2)  # 2 表示警报
                self.paly_sound()
            else:
                self.label_12.setStyleSheet("color:black;")
            

#modify here, if needed can be justify               
            if hasattr(self, 'share_data'):
                
                self.shared_data.update_status(d['value'][1])
#modify part end
